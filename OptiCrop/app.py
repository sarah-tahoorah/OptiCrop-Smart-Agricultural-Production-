...
def save_prediction(inputs, crop_key, confidence, model_name):
    ...
    cursor.execute(
        ...
        (1, prediction_id, "Crop Recommendation", now),
    )
    connection.commit()
    connection.close()
...
def save_suitability(inputs, crop_key, analysis, prediction_id=None):
    ...
    if prediction_id is None:
        cursor.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (1, crop_id, inputs["N"], inputs["P"], inputs["K"], inputs["temperature"], inputs["humidity"], inputs["ph"], inputs["rainfall"], analysis["score"], "Suitability Checker", now),
        )
        prediction_id = cursor.lastrowid
    cursor.execute(
        """
    ...
    cursor.execute(
        "INSERT INTO prediction_history (user_id, prediction_id, action_type, action_at) VALUES (?, ?, ?, ?)",
        (1, prediction_id, "Suitability Analysis", now),
    )
    connection.commit()
    connection.close()
...
def contact():
    if request.method == "POST":
        ...
        connection.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
            """,
            (1, name, email, subject, message, rating, datetime.utcnow().isoformat()),
        )
        connection.commit()
        connection.close()
